...
transform_A_g = A.Compose(
    [   
        A.OneOf([
                A.RandomCrop(150, 150, always_apply=True),
                A.RandomCrop(200, 200, always_apply=True),
                A.RandomCrop(250, 250, always_apply=True),
                A.RandomCrop(300, 300, always_apply=True),
                A.RandomCrop(350, 350, always_apply=True),
                A.RandomCrop(400, 400, always_apply=True),
                A.RandomCrop(450, 450, always_apply=True),
...
transform_A_g_strong = A.Compose(
    [   
        # A.Cutout(num_holes=3, max_h_size=100, max_w_size=100, always_apply=False,p=0.4),
        # A.HorizontalFlip(always_apply=False,p=0.4),
        # A.VerticalFlip(always_apply=False,p=0.4),
        # A.OneOf([
        ...
        #          mask_value=12, rotate_method='largest_box', crop_border=False, always_apply=False,p=0.4),
        # A.RandomSizedCrop(min_max_height=[512,1024], height=args.resize, width=args.resize,  #고려해보아야함
                        #    w2h_ratio=1.0, interpolation=1, always_apply=False,p=0.5),
        #A.Cutout(always_apply=True, p=0.5, num_holes=5, max_h_size=28, max_w_size=28),
        A.OneOf([
...
#checkpoint = torch.load('./out/test_ckpt_save/Segformer_lr0.001_wd0.0005_lu0.5_25epoch.pt')
student_model = torch.load('./out/test_ckpt_save/Segformer_lr0.001_wd0.0005_lu0.5_25epoch.pt',map_location='cuda')
teacher_model = torch.load('./out/test_ckpt_save/Segformer_lr0.001_wd0.0005_lu0.5_25epoch.pt',map_location='cuda')
...
student_model.load_state_dict(torch.load('./out/test_ckpt_save/Segformer_lr0.001_wd0.0005_lu0.5_25epoch.pt'))
teacher_model.load_state_dict(torch.load('./out/test_ckpt_save/Segformer_lr0.001_wd0.0005_lu0.5_25epoch.pt'))
    ...
    if epoch < warmup_epoch:
        ...
        for i, data in enumerate(zip(tqdm(dataloader), target_loader)):
            ...
            l_u, pseudo_high_ratio = compute_unsupervised_loss_by_threshold(pred_u_strong, p_t.detach(), p_t_logit.detach(), criterion=ce_loss,
                                                                            thresh=0.9, ignore_index=args.ignore)
